...
The only difference - WinSparkle does not support DSA keys larger than 1024 (because WinAPI does not).
You should only use DSA keys generated by WinSparkle version of the tool `generate_keys.bat`.
...
Alternatively, you can sing your updates even on macOS or Linux,
using [sign_update tool provided by Sparkle project](https://github.com/sparkle-project/Sparkle/tree/master/bin).

NOTE: Do not use `generate_keys` from Sparkle project to generate keys to sign your Windows update, as this keys are not compatible with WinSparkle.
